...
feature -- parsing
	...
	evt_to_cmd (evt : TUPLE[name: STRING; args: ARRAY[ETF_EVT_ARG]]) : ETF_COMMAND_INTERFACE
		...
		do
			...
			elseif cmd_name ~ "custom_setup" then 
				 if attached {ETF_INT_ARG} args[1] as dimension and then 4 <= dimension.value and then dimension.value <= 12 and then attached {ETF_INT_ARG} args[2] as ships and then 1 <= ships.value and then ships.value <= 7 and then attached {ETF_INT_ARG} args[3] as max_shots and then 1 <= max_shots.value and then max_shots.value <= 144 and then attached {ETF_INT_ARG} args[4] as num_bombs and then 1 <= num_bombs.value and then num_bombs.value <= 7 then 
	...
	do
		...
		loop
			...
			elseif cmd_name ~ "custom_setup" then 
				if NOT( ( args.count = 4 ) AND THEN attached {ETF_INT_ARG} args[1] as dimension and then 4 <= dimension.value and then dimension.value <= 12 and then attached {ETF_INT_ARG} args[2] as ships and then 1 <= ships.value and then ships.value <= 7 and then attached {ETF_INT_ARG} args[3] as max_shots and then 1 <= max_shots.value and then max_shots.value <= 144 and then attached {ETF_INT_ARG} args[4] as num_bombs and then 1 <= num_bombs.value and then num_bombs.value <= 7) then 
					if NOT Result.is_empty then
						Result.append ("%N")
					end
					Result.append (evt_out_str + " does not conform to declaration " +
							"custom_setup(dimension: GRID_SIZE = 4 .. 12 ; ships: NUMBER_OF_SHIPS = 1 .. 7 ; max_shots: MAX_SHOTS = 1 .. 144 ; num_bombs: NUMBER_OF_BOMBS = 1 .. 7)")
				end
			...
			elseif cmd_name ~ "custom_setup_test" then 
				if NOT( ( args.count = 4 ) AND THEN attached {ETF_INT_ARG} args[1] as dimension and then 4 <= dimension.value and then dimension.value <= 12 and then attached {ETF_INT_ARG} args[2] as ships and then 1 <= ships.value and then ships.value <= 7 and then attached {ETF_INT_ARG} args[3] as max_shots and then 1 <= max_shots.value and then max_shots.value <= 144 and then attached {ETF_INT_ARG} args[4] as num_bombs and then 1 <= num_bombs.value and then num_bombs.value <= 7) then 
					...
					Result.append (evt_out_str + " does not conform to declaration " +
							"custom_setup_test(dimension: GRID_SIZE = 4 .. 12 ; ships: NUMBER_OF_SHIPS = 1 .. 7 ; max_shots: MAX_SHOTS = 1 .. 144 ; num_bombs: NUMBER_OF_BOMBS = 1 .. 7)")
				end
